...
const findings = [
   {
      ...
      desc: 'Ağır JavaScript yürütümü nedeniyle kaydırma sırasında TBT (Total Blocking Time) 850ms seviyesin...',
      tag: 'Kritik',
   ...
   },
   {
      ...
      desc: 'Navigasyon menüsündeki linkler 48×48px erişilebilirlik standartlarını karşılamıyor. Tıklama hataları oranı %12...',
      tag: 'Uyarı',
   ...
   },
   {
      ...
      desc: 'Kahraman (Hero) görseli yeni nesil formatlarda (WebP/AVIF) sunulmuyor. LCP metriklerini olumsuz...',
      tag: 'Bilgi',
      ...
      <header className="border-b border-white/10 bg-[#0d0f0f]">
         <div className="flex h-14.5 items-center justify-between px-10 max-lg:px-5 max-md:h-auto max-md:flex-col max-md:items-start max-md:gap-4 max-md:py-4">
            <div className="flex items-center gap-9 max-md:flex-col max-md:items-start max-md:gap-4">
               <Link href="/" className="text-[23px] font-bold tracking-[-0.8px] text-[#70f8ff]">
                  AetherAnalytics
               </Link>

               <nav className="flex items-center gap-6 max-md:w-full max-md:overflow-auto">
                  {topNav.map((item) => (
                     <Link key={item.title} href={item.href} className={`relative shrink-0 text-[14px] font-medium tracking-[0.2px] transition ${item.title === 'Scanner' ? 'text-[#70f8ff]' : 'text-[#b3bcbb] hover:text-white'}`}>
                        {item.title}

                        {item.title === 'Scanner' && <span className="absolute -bottom-2.5 left-0 h-px w-full bg-[#70f8ff]" />}
                     </Link>
                  ))}
               </nav>
            </div>
            ...
            <div className="flex items-center gap-5 max-md:w-full max-md:justify-between">
               ...
               <Link href="/scanner" className="flex h-7.75 items-center justify-center rounded-xs bg-linear-to-r from-[#19dce9] to-[#5524d5] px-5 text-[12px] font-bold tracking-[0.4px] text-white">
                  Execute Analysis
               </Link>
      ...
      <aside className="flex min-h-[calc(100vh-64px)] flex-col border-r border-white/10 bg-[#0b0d0d] max-lg:hidden">
         <div className="border-b border-white/10 px-8 py-7">
            <div className="flex items-center gap-3">
               ...
               <div>
                  <h2 className="text-[23px] font-bold tracking-[-0.8px] text-[#6ff8ff]">AetherOS v1.0</h2>
                  <p className="font-mono text-[12px] font-bold text-[#a0aaa9]">System Nominal</p>
               </div>
            </div>

            <a href="/scanner" className="mt-6 flex h-6.75 w-full items-center justify-center rounded-xs bg-white text-[12px] font-bold text-[#111]">New Scan</a>
         </div>

         <nav className="py-8">
            {sidebarItems.map((item) => (
               <a key={item.title} href="#" className={`relative flex h-14 items-center gap-6 px-8 font-mono text-[12px] font-bold tracking-[0.6px] transition ${item.active ? 'bg-[#24113e] text-[#17dce9]' : 'text-[#aeb8b7] hover:bg-white/4 hover:text-white'}`}>
                  <span>{item.icon}</span>
                  {item.title}

                  {item.active && <span className="absolute right-0 top-0 h-full w-0.75 rounded-full bg-[#70f8ff]" />}
               </a>
            ))}
         </nav>
         ...
         <div className="mt-auto border-t border-white/10 px-8 py-8">
            <div className="space-y-8">
               <a href="#" className="flex items-center gap-5 font-mono text-[12px] font-bold text-[#b9c2c1]">
                  <HelpIcon />
                  Support
               </a>
         ...
         <div className="mx-auto max-w-325">
            <div className="mb-9 flex items-end justify-between gap-6 max-md:flex-col max-md:items-start">
               <div>
                  ...
                  <h1 className="text-[44px] font-bold leading-none tracking-[-2px] text-[#f0eeee] max-md:text-[34px]">Tam Denetim Raporu</h1>

                  <p className="mt-5 max-w-150 text-[18px] font-medium leading-normal text-[#b9c4c3]">UX ve Performans anormalliklerinin detaylı analizi. Yapay zeka destekli çözüm önerileri üretilmiştir.</p>
               </div>
               ...
               <div className="mb-1 flex items-center gap-2">
                  ...
                  <button className="h-8.5 bg-[#15dbe8] px-5 text-[12px] font-bold text-[#042f32] transition hover:bg-[#77faff]">Geliştiricilere İlet</button>
               </div>
            </div>
   ...
   return (
      <div className="relative overflow-hidden rounded-md border border-white/10 bg-[#080909] p-6">
         ...
         <div className="relative z-10">
            <h2 className="text-[22px] font-bold tracking-[-0.6px] text-[#e5dddd]">Sistem Sağlık Skoru</h2>

            <div className="mt-11 flex justify-center">
               <div className="relative flex size-34.5 items-center justify-center rounded-full bg-[conic-gradient(#ffaaa4_0deg_210deg,#536363_210deg_360deg)]">
                  <div className="absolute size-29.5 rounded-full bg-[#050707]" />

                  <div className="relative text-center">
                     <div className="text-[33px] font-bold leading-none text-[#ffaaa4]">58</div>
                     <div className="mt-1 text-[12px] font-bold text-[#9fa9a8]">/100</div>
                  </div>
               </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 text-center">
               <ScoreMini title="Performans" value="42" danger />
               <ScoreMini title="UX" value="76" />
               <ScoreMini title="Güvenlik" value="94" cyan />
            </div>
         </div>
      </div>
   )
...
function ScoreMini({ title, value, danger, cyan }: { title: string; value: string; danger?: boolean; cyan?: boolean }) {
   return (
      <div>
         <p className="font-mono text-[10px] font-bold text-[#84908f]">{title}</p>
         <p className={`mt-2 text-[17px] font-bold ${danger ? 'text-[#ffaaa4]' : cyan ? 'text-[#18dce9]' : 'text-[#5df6a8]'}`}>{value}</p>
      </div>
   )
}
   ...
   return (
      <div className="rounded-md border border-white/10 bg-[#070808] p-6">
         ...
         <div className="space-y-3">
            {findings.map((item) => (
               <div key={item.title} className="flex items-center gap-5 rounded-[3px] bg-[#1b1c1c] px-4 py-4">
                  <span className={`flex size-9 shrink-0 items-center justify-center ${item.iconBg}`}>{item.icon}</span>

                  <div className="min-w-0 flex-1">
                     <h3 className="truncate text-[17px] font-bold text-[#d6d0d0]">{item.title}</h3>

                     <p className="mt-1 line-clamp-2 text-[13px] font-medium leading-[1.45] text-[#9ea8a7]">{item.desc}</p>
                  </div>

                  <span className={`shrink-0 rounded-xs px-3 py-2 font-mono text-[12px] font-bold ${item.color}`}>{item.tag}</span>
               </div>
            ))}
         </div>
      </div>
      ...
      <div className="mt-6 rounded-md border border-white/10 bg-[#070808] p-6">
         ...
         <div className="grid grid-cols-2 gap-8 max-lg:grid-cols-1">
            <div className="border-l-2 border-[#ffaaa4] pl-5">
               <h3 className="text-[16px] font-bold text-[#d6d0d0]">Web Worker Entegrasyonu</h3>

               <p className="mt-3 text-[14px] font-medium leading-normal text-[#aab4b3]">Ağır veri işleme görevlerini ana iplikten ayırmak için Web Worker kullanın. Bu, TBT&apos;yi tahmini olarak %80 azaltacaktır.</p>
...
worker.postMessage(heavyDataset);`}
            ...
            <div className="border-l-2 border-[#18dce9] pl-5">
               <h3 className="text-[16px] font-bold text-[#d6d0d0]">Erişilebilirlik Düzenlemeleri</h3>

               <p className="mt-3 text-[14px] font-medium leading-normal text-[#aab4b3]">Navigasyon elemanlarına `min-h-[48px]` ve `min-w-[48px]` uygulayarak dokunmatik hedef alanlarını genişletin.</p>
...
function VitalsTable() {
   return (
      <div className="mt-6 overflow-hidden rounded-md border border-white/10 bg-[#070808]">
         <h2 className="px-6 py-6 text-[22px] font-bold tracking-[-0.8px] text-[#e5dddd]">Rakip Kıyaslaması (Core Web Vitals)</h2>
         ...
         <div className="overflow-auto">
            <table className="w-full min-w-190 text-left">
               ...
               <tbody>
                  {vitals.map((item) => (
                     <tr key={item.metric} className="border-t border-white/6 text-[14px] font-bold text-[#aeb8b7]">
                        <td className="px-4 py-5">{item.metric}</td>
                        <td className={`px-4 py-5 font-mono ${item.status === 'warning' ? 'text-[#ffaaa4]' : item.metric.includes('CLS') ? 'text-[#18dce9]' : 'text-[#5df6a8]'}`}>{item.ours}</td>
                        <td className="px-4 py-5 font-mono text-[#7d8786]">{item.average}</td>
                        <td className="px-4 py-5 font-mono text-[#8f9998]">{item.leader}</td>
                        <td className="px-4 py-5">
                           <div className="flex justify-end">
                              {item.status === 'warning' ? (
                                 ...
                                 <span className={item.metric.includes('CLS') ? 'text-[#18dce9]' : 'text-[#5df6a8]'}>
                                    <CheckCircleIcon />
                                 </span>
                              )}
                           </div>
